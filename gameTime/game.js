...
this.anims.create({
    ...
    //Adding portal image on last platform
    this.portals = this.physics.add.staticGroup(); 
    const portal = this.portals.create(9600,440, 'portal');
                portal.setScale(1);
    ...
    this.physics.add.collider(this.player, this.missiles, gameOver, null, this, );
...
this.physics.add.collider(this.player, this.portals, gameWin, null, this);


gameWinText = this.add.text(game.config.width / 2, game.config.height / 2, 'Winner!', {
    fontSize: '124px',
    fontFamily: 'Arial',
    color: '#ffffff'
}).setOrigin(0.5);
...
function showGameWinText() {
    gameWinText.visible = true;
    gameWinText.x = this.cameras.main.scrollX + game.config.width / 2;
    gameWinText.y = this.cameras.main.scrollY + game.config.height / 2;
}
